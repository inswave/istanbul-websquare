...
var istanbul = require('istanbul'),
    ...
    baselineCoverage = {};
global.coverageObjList = [];

//single place to get global coverage object
function getCoverageObject(browser) {
    /*jslint nomen: true */
    if(!browser) {
        return;
    }
    global.coverageObjList["all"] = {};
    if(browser == "all") {
        for(var bName in global.coverageObjList) {
            if(bName == "all") {
                continue;
            }
            obj = global.coverageObjList[bName];
            Object.keys(obj).forEach(function (filePath) {
                var original = global.coverageObjList["all"][filePath],
                    added = obj[filePath],
                    result;
                ...
                global.coverageObjList["all"][filePath] = result;
            });
        }
    }
    global.coverageObjList[browser] = global.coverageObjList[browser] || {};
    return global.coverageObjList[browser];
}

//set global coverage object(Danger function)
function setCoverageObject(coverageObject, browser) {

    if(!browser) {
        return;
    }
    global.coverageObjList[browser] = coverageObject || {};
    return;
}
 ...
 */
function restoreBaseline(browser) {
    browser = browser || 'chrome';
    var cov = getCoverageObject(browser),
        fileCoverage,
        fileBaseline;
...
function render(filePath, res, prefix, browser) {
    browser = browser || 'all';
    var collector = new istanbul.Collector(),
        ...
        report,
        fileCoverage,
        coverage = getCoverageObject(browser);
    ...
    if (browser == 'all') {
        ...
        linkMapper = {
            hrefFor: function (node) {
                return prefix + 'show/' + browser +'?p=' + node.fullPath();
            },
            fromParent: function (node) {
                return this.hrefFor(node);
            },
...
function mergeClientCoverage(obj, browser) {
    browser = browser || "chrome";
    if (!obj) { return; }
    var coverage = getCoverageObject(browser);
    var excludeFile = config.excludeFileList;
    ...
    Object.keys(obj).forEach(function (filePath) {
        ...
        var splitList = filePath.split("/");
        var fileName = splitList[splitList.length - 1];
        if(!!includePath && !filePath.includes(includePath)) {
            return;
        }
        ...
        if(!!excludePath && !!excludePath.length) {
            for(var i=0; i<excludePath.length; i++) {
                if(filePath.includes(excludePath[i])) {
                    return;
                }
            }
        }
...
module.exports = {
    ...
    setCoverageObject: setCoverageObject,
